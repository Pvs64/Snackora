...
function App() {
  ...
  return (
    <>
      <CursorGlow />
      <div className="min-h-screen bg-backgroundLight text-[#40210f] dark:bg-backgroundDark dark:text-white transition-all duration-500">
        <header className="sticky top-0 z-50 backdrop-blur-xl bg-white/20 dark:bg-black/20 border-b border-white/10 dark:border-black/10 shadow-sm transition-all duration-300">
          <div className="container mx-auto px-4 py-3 flex items-center justify-between">
            <Link to="/" className="flex items-center">
              <img src={logo} alt="Snackora" className="h-[300px] w-[400px]  ml-[-50px] mt-[-120px] mb-[-170px]" />
            </Link>

            <div className={`relative flex-1 max-w-xl mx-4 transition-all duration-300 ${isSearchFocused ? 'scale-[1.02]' : ''}`}>
          ...
          >
            ...
            >
              ...
              <form
                onSubmit={(e) => {
                  e.preventDefault();
                  console.log(`${loginData.mode === 'login' ? 'Logging in' : 'Signing up'} with`, loginData);
                  setShowLogin(false);
                }}
                className="space-y-4"
              >
                ...
                {loginData.mode === 'signup' && (
                  <div>
                    ...
                    <input
                      ...
                      name="confirm"
                      required
                      onChange={(e) => setLoginData(prev => ({ ...prev, confirm: e.target.value }))}
                      className="w-full px-4 py-2 rounded-lg border border-gray-300 dark:border-gray-600 focus:ring-2 focus:ring-orange-500 dark:bg-gray-700 dark:text-white transition-all"
                      placeholder="••••••••"
                    />
                  </div>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <a href="#" className="text-sm text-orange-500 dark:text-orange-400 hover:underline">Forgot password?</a>
                </div>
